...
pushd $dump_dir
    ...
    if [ ! -f stu.txt ];
        then reject-solution "stu.txt is missing! Try again.";
    fi

    if [ ! -f steiner.txt ];
        then reject-solution "steiner.txt is missing! Try again.";
    fi
popd
...
if [[ $number_of_files_changed -ne 2 ]]
    then reject-solution "More than 2 files were changed! Only add stu.txt and steiner.txt. Check out the original branch and try again.";
fi
